                ...
                for page in tqdm(range (num_pages),desc="Page",position=2,leave=False):
                    ...
                    for i in range(len(page_image_paths)):
                        ...
                        if normalize:
                            ...
                            image = T(cv2.cvtColor(cv2.imread(image_path), cv2.COLOR_BGR2RGB))
...
def postprocess(image_tensor): 
    return transforms.functional.convert_image_dtype(image_tensor, torch.uint8).squeeze().detach().cpu().permute(1, 2, 0).numpy() # type: ignore
def postprocess_batch(image_tensor):
    return transforms.functional.convert_image_dtype(image_tensor, torch.uint8).detach().cpu().permute(0, 2, 3, 1).numpy() # type: ignore

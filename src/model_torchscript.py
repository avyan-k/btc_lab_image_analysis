...
if __name__ == "__main__":
    ...
    traced_resnet_classifier.load_state_dict(torch.load('./results/training/models/ResNet_Tumor/DDC_UC_1/4-257-0.9653239250183105.pt', map_location = DEVICE,weights_only=True))

    model_path = f"./results/training/torchscript_models/{str(type(resnet_classifier).__name__)}/{tumor_type}"
    Path(model_path).mkdir(parents=True, exist_ok=True)
    traced_resnet_classifier.save(model_path)

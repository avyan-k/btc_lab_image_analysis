...
def get_torchscript_resnet_tumor(tumor_type, weight_path):
    ...
    feature_classifier = md.Tumor_Classifier(layers=5, neurons_per_layer=64, dropout=0.0, input_neurons=1000, classes=len(classes)) 
    traced_feature_classifier = torch.jit.script(feature_classifier)
    resnet_classifier = md.ResNet_Tumor(classes=len(classes),feature_classifier=traced_feature_classifier)
    ...
    traced_resnet_classifier.load_state_dict(torch.load(weight_path, map_location = torch.device('cpu'), weights_only=True))
...
def save_resnettumor_model(path,config,model_path):
    ...
    torch.jit.save(traced_resnet_classifier,os.path.join(model_path,"torchscript_model.pt"))
...
if __name__ == "__main__":
    ...
    for tumor_type in os.listdir('./images'):
        ...
        if tumor_type in ['.DS_Store','__MACOSX','SIL'] :
            continue
        norm_weight_path = f'./results/training/models/ResNet_Tumor/{tumor_type}-Normalized.pt'
        unnorm_weight_path = f'./results/training/models/ResNet_Tumor/{tumor_type}-Unnormalized.pt'
        for weight_path in [norm_weight_path,unnorm_weight_path]:
            ...
            traced_resnet_classifier = get_torchscript_resnet_tumor(tumor_type,weight_path)
            config = generate_config_json_resnettumor(tumor_type)
            model_type = os.path.basename(os.path.splitext(weight_path)[0])
            model_path = f"./results/training/torchscript_models/ResNet_Tumor/{model_type}/"
            print(model_path)

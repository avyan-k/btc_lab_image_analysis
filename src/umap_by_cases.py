...
def generate_umap_cases(embeddings, cases, tumor_type, outfile):
    ...
    handles = [Line2D([0], [0], marker='o', color='w', markerfacecolor=case_colors[i], markersize=10, label=f'Case {i+1}') for i in range(num_cases)]
    plt.legend(handles=handles, title='Cases', bbox_to_anchor=(0.5, -0.1), loc='upper center', ncol=num_cols_in_legend, fontsize='small')
    
    plt.title(f'{tumor_type} UMAP Projection of UMAP embeddings - Cases')
    plt.xlabel('UMAP Dimension 1')
    plt.ylabel('UMAP Dimension 2')
    plt.subplots_adjust(bottom=0.25)  # Increase the bottom margin to fit the legend without squishing the plot
...
if __name__ == "__main__":
    ...
    # Files
    results_directory = f"./results/umap-cases"
    Path(os.path.join(results_directory)).mkdir(parents=True, exist_ok=True)
    umap_cases_file = f"umap-cases_{tumor_type}_{model_type}_{run_id}_{seed}_{sample_size}.png"
    ...
    generate_umap_cases(embeddings=umap_embeddings, cases=cases, tumor_type=tumor_type, outfile=umap_cases_output_path)
...
class BalancedTumorImageData(TumorImageDataset):
    def __init__(self, datapath: str, k: int, transform=None, target_transform=None, cases=None):
        ...
        """
        super().__init__(root=datapath, transform=transform, target_transform=target_transform, cases=cases)
        self.k = k
...
def get_image_dataset(
    ...
    if samples_per_class == -1:
        ...
        return BalancedTumorImageData(image_directory, k=samples_per_class, transform=processing_transforms)
...
def split_datasets(
    ...
    train_classes = dict(
        sorted(Counter([dataset.targets[i] for i in train_dataset.indices]).items())
    )  # counter return a dictionnary of the counts, sort and wrap with dict to get dict sorted by key
    test_classes = dict(sorted(Counter([dataset.targets[i] for i in test_dataset.indices]).items()))
    valid_classes = None
    if valid_dataset is not None:
        valid_classes = dict(sorted(Counter([dataset.targets[i] for i in valid_dataset.indices]).items()))
...
def load_training_image_data(
    ...
    if validation:
        ...
        return get_loaders_training_image_data(
            ...
            train_class,
            test_class,
            {k: v for k, v in enumerate(full_dataset.classes)},
            valid_set,
            valid_class,
...
def get_loaders_training_image_data(
    ...
    if weighted:
        train_sampler = create_weighted_sampler(
            train_dataset, train_classes, max(train_classes.values()), True, label2class
        )
        ...
        test_sampler = create_weighted_sampler(
            test_dataset, test_classes, max(test_classes.values()), True, label2class
        )
...
def load_image_data(
    ...
    """
    full_dataset = get_image_dataset(
        tumor_type=tumor_type, samples_per_class=samples_per_class, stain_normalized=stain_normalize, verbose=True
    )
    ...
    classes = dict(
        sorted(Counter(full_dataset.targets).items())
    )  # counter return a dictionnary of the counts, sort and wrap with dict to get dict sorted by key
...
def load_training_image_data_by_case(batch_size, tumor_type, transforms=None, normalized=False):
    ...
    transforms = (
        v2.Compose([v2.RandomAffine(degrees=(-15, 15), translate=(0.15, 0.15)), transforms])
        if transforms is not None
        else v2.RandomAffine(degrees=(-15, 15), translate=(0.15, 0.15))
    )
    ...
    training_valid_dataset = TumorImageDataset(root=image_directory, cases=train_subset, transform=transforms)
    test_dataset = TumorImageDataset(root=image_directory, cases=test_subset, transform=transforms)
    ...
    train_classes = dict(
        sorted(Counter([training_valid_dataset.targets[i] for i in train_dataset.indices]).items())
    )  # counter return a dictionnary of the counts, sort and wrap with dict to get dict sorted by key
...
def get_image_directory(tumor_type, stain_normalized):
    image_directory = (
        f"./images/{tumor_type}/images" if not stain_normalized else f"./images/{tumor_type}/normalized_images"
    )
...
def create_weighted_sampler(
    dataset: datasets.DatasetFolder | torch.utils.data.Subset,
    class_dict,
    samples_per_class,
    verbose=False,
    label2class=None,
    ...
    if verbose:
        ...
        for tumor_class, class_weight in class_dict.items():
            if class_weight < samples_per_class:
                ...
                print(
                    f"Found {class_weight}  samples for class {label2class[tumor_class]}, more than the requested {samples_per_class}. It will be downsampled (some samples will be skipped) during training"
                )
            else:
                print(
                    f"Found {class_weight} samples for class {label2class[tumor_class]}, as many as the requested {samples_per_class}. All samples will be equally used."
                )
        ...
        for image_path in tqdm(os.listdir(os.path.join(image_directory, annotation))):
            ...
            if image.shape == (512, 512, 3):
                for idx, tile in enumerate(
                    [image[x : x + 256, y : y + 256] for x in range(0, 512, 256) for y in range(0, 512, 256)]
                ):
                    # print(os.path.splitext(image_full_path)[0]+f"_tile_{idx+1}"+os.path.splitext(image_full_path)[1])
                    imwrite(
                        os.path.splitext(image_full_path)[0] + f"_tile_{idx+1}" + os.path.splitext(image_full_path)[1],
                        tile,
                    )
...
def find_cases(image_directory):
    ...
    case_dict = {
        case: sorted([os.path.join(path.parent, path.name) for path in paths if case in path.name]) for case in cases
    }
    return case_dict
...
def get_mean_std_per_channel(dataset, dataset_info, seed):
    ...
    """
    mean_std_path = f"./results/training/mean_stds/{dataset_info}"
    mean_std_path += ".txt"
    try:
        ...
        means, stds = compute_and_save_mean_std_per_channel(dataset=dataset, path=mean_std_path, seed=seed)
    return means, stds

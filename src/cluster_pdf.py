...
def save_clusters_to_pdf(cluster_info, outfile):
    ...
    with PdfPages(outfile) as pdf:
        ...
        for cluster in unique_clusters:
            ...
            cluster_images_info = cluster_info[
                cluster_info["Cluster"] == cluster
            ]  # subset of cluster_info
            image_paths = cluster_images_info[
                "ImagePath"
            ].tolist()  # paths for the images in the cluster
...
def get_clusters(
    cluster_type, embeddings, seed, n_clusters=10, eps=0.4, min_samples=500
):
    """
    ...
    if cluster_type == "k-means":
        ...
        clusters = agglomerative_clustering_no_n_clusters(
            embeddings, distance_threshold=distance_threshold
        )
        ...
        n_clusters = len(
            set(clusters)
        )  # note: DBSCAN assigns -1 to noise points, this will still be considered as first cluster
    else:
        print(
            f"cluster_type: {cluster_type} is invalid. dbscan clustering is used with eps=0.4 and min_samples=500 (ddc-uc settings)."
        )
        clusters = dbscan_clustering(
            umap_embeddings, eps=eps, min_samples=min_samples
        )  # ddc_uc settings
...
def plot_umap_for_kmeans(cluster_type, n_clusters, clusters, umap_embeddings, outfile):
    ...
    plt.title(
        f"{tumor_type} UMAP Projection with {cluster_type} clustering on UMAP embeddings"
    )
...
def save_cluster_csv(
    image_paths, cases, annotations, clusters, save_csv=False, outfile=""
):
...
if __name__ == "__main__":
    ...
    results_directory = "./results/clusters"
    ...
    plot_umap_for_kmeans(
        cluster_type=cluster_type,
        n_clusters=n_clusters,
        clusters=clusters,
        umap_embeddings=umap_embeddings,
        outfile=umap_kmeans_output_path,
    )

    cluster_info = save_cluster_csv(
        image_paths,
        get_cases(image_paths),
        annotations,
        clusters,
        save_csv=True,

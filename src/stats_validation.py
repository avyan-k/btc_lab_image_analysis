...
def create_cluster2pred_mapping(n_clusters, tumor_type):
    ...
    for i in range(-1, n_clusters-1):
        while True: # will break if valid input is given, otherwise stays in loop to ask for input again
            pred = input(f"Cluster {i} is: ").strip().lower() # gets which class each cluster corresponds to (normal/tumor (or undiff, well_diff), noise, exclude)
            if "DDC_UC" in tumor_type:
                if pred in ['normal', 'undiff', 'well_diff', 'noise', 'exclude']: # checks if input is valid
                    cluster_to_pred[i] = pred
                    break
                else:
                    print("Invalid input. Please enter 'normal', 'undiff', 'well_diff', 'noise' or 'exclude'.")
            else:
                if pred in ['normal', 'tumor', 'noise', 'exclude']:
                    cluster_to_pred[i] = pred
                    break
                else:
                    print("Invalid input. Please enter 'normal', 'tumor', 'noise' or 'exclude'.")
...
def stats_validation(csv_file, n_clusters, tumor_type, output_file=None):
    ...
    cluster_to_pred = create_cluster2pred_mapping(n_clusters, tumor_type) # maps cluster label to predicted class
...
if __name__ == "__main__":
    ...
    output_file = f"./results/clusters/umap_SCCOHT_1_ResNet_dbscan_2024-09-21_99_good_prediction.csv"
